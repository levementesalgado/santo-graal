...
export const generateRegionalRecommendations = (state: string, data: ConabRecord[]): string[] => {
...
export const analyzeSeasonalTrends = (data: ConabRecord[]): SeasonalTrend => {
  const sorted = [...data].sort((a, b) => a.year - b.year)
  const changes: number[] = sorted.map((d, i) => {
    if (i === 0) return 0
...
export const detectAnomalies = (data: ConabRecord[]): ConabRecord[] => {
