...
import {
  LayoutDashboard, BarChart3, Map, BrainCircuit, Database,
  Menu, X, Search, Cpu, Terminal, Layers
} from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col font-sans">
            ...
            <div className="flex items-center gap-3 mb-3">
              ...
              <div className="flex flex-col">
                ...
                <span className="text-[10px] text-primary flex items-center gap-1">
                  <span className="w-1.5 h-1.5 bg-primary rounded-full animate-pulse" /> Operacional
                </span>
              </div>
            </div>
          </div>
      ...
      <div className="lg:pl-64 flex flex-col min-h-screen">
        <header ref={headerRef} className="sticky top-0 z-30 w-full border-b border-border bg-background/80 backdrop-blur-md">
          <div className="flex items-center justify-between px-6 py-4">
            <div className="flex items-center gap-4 flex-1">
              ...
              <div className="relative max-w-md w-full hidden md:block">
                ...
                <Input placeholder="Pesquisar safras, estados ou modelos..." className="pl-10 bg-muted/30 border-none focus-visible:ring-primary rounded-xl" />
              </div>
            </div>
            <div className="hidden sm:flex items-center gap-2 mr-4">
              <Select defaultValue="2026">
                <SelectTrigger className="w-[100px] bg-muted/30 border-none rounded-lg h-9"><SelectValue placeholder="Ano" /></SelectTrigger>
                <SelectContent>
              ...
              <Select defaultValue="nacional">
                <SelectTrigger className="w-[140px] bg-muted/30 border-none rounded-lg h-9"><SelectValue placeholder="Região" /></SelectTrigger>
                <SelectContent>
        ...
        <main className="flex-1 p-6 md:p-8" style={{ minHeight: `calc(100vh - ${headerHeight}px)` }}>
        ...
        <footer className="border-t border-border bg-muted/10 py-8 px-8">
          <div className="flex flex-col md:flex-row justify-between items-center gap-6">
            ...
            <div className="flex flex-wrap justify-center gap-8 text-xs font-mono">
              <div className="flex flex-col gap-1"><span className="text-muted-foreground uppercase">Latência de Cálculo</span><span className="font-bold">12.4ms</span></div>
              <div className="flex flex-col gap-1"><span className="text-muted-foreground uppercase">Integridade dos Dados</span><span className="text-primary font-bold">99.98%</span></div>
              <div className="flex flex-col gap-1"><span className="text-muted-foreground uppercase">Última Extração</span><span className="font-bold">21-02-2026 02:33</span></div>
            </div>
          </div>
          ...
          <>
            ...
            <motion.div initial={{ x: "-100%" }} animate={{ x: 0 }} exit={{ x: "-100%" }}
              ...
              <div className="flex items-center justify-between mb-8">
                <div className="flex items-center gap-2"><Cpu className="text-primary w-6 h-6" /><span className="font-bold text-lg">AgroPrecision</span></div>
                <Button variant="ghost" size="icon" onClick={() => setIsMobileMenuOpen(false)}><X className="w-6 h-6" /></Button>
              </div>
