    ...
    <div className="space-y-4">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        {filterable && (
          <div className="relative w-full md:w-96 group">
            ...
            <Input placeholder="Pesquisar por UF, Cultura ou Região..." value={search}
              onChange={(e) => setSearch(e.target.value)}
              className="pl-10 bg-card border-border rounded-xl focus-visible:ring-primary/20" />
          </div>
        )}
        {exportable && (
          <Button variant="outline" onClick={exportToCSV} className="rounded-xl border-border hover:bg-muted gap-2 w-full md:w-auto">
        ...
        <Table>
          <TableHeader className="bg-muted/50">
            <TableRow className="hover:bg-transparent border-border">
              {columns.map(col => (
                <TableHead key={col} className="font-semibold text-foreground cursor-pointer select-none" onClick={() => handleSort(col)}>
                  <div className="flex items-center gap-2">{COLUMN_LABELS[col] || col}<ArrowUpDown className="w-3 h-3 opacity-50" /></div>
                </TableHead>
              ))}
            </TableRow>
          </TableHeader>
          <TableBody>
            <AnimatePresence mode="wait">
              {paginatedData.map(item => (
                <motion.tr key={item.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.98 }}
      ...
      <div className="flex items-center justify-between px-2">
        ...
        <div className="flex gap-2">
          <Button variant="outline" size="sm" disabled={currentPage === 1} onClick={() => setCurrentPage(p => p - 1)} className="rounded-lg">Anterior</Button>
          <div className="flex items-center gap-1 px-2">
            <span className="text-sm font-medium">{currentPage}</span>
            <span className="text-sm text-muted-foreground">/ {totalPages || 1}</span>
          </div>
          <Button variant="outline" size="sm" disabled={currentPage >= totalPages} onClick={() => setCurrentPage(p => p + 1)} className="rounded-lg">Próximo</Button>
        </div>
      </div>
    </div>
  );
        ...
        <TableBody>
          {metrics.map(m => (
            <TableRow key={m.state} className="border-border">
              ...
              <TableCell>
                {m.trend === 'up'
                  ? <Badge className="bg-primary/10 text-primary border-primary/20 gap-1"><TrendingUp className="w-3 h-3" /> Alta</Badge>
                  : <Badge className="bg-destructive/10 text-destructive border-destructive/20 gap-1"><TrendingDown className="w-3 h-3" /> Queda</Badge>}
              </TableCell>
            </TableRow>
          ))}
        </TableBody>
        ...
        {states.map(state => (
          <div key={state} className="p-4 rounded-2xl border border-border bg-card shadow-sm">
            <h3 className="text-lg font-bold mb-4 flex items-center gap-2">
              <div className="w-2 h-2 rounded-full bg-primary" /> Análise Comparativa: {state}
            </h3>
            <Table>
              <TableHeader>
                <TableRow><TableHead>Ano</TableHead><TableHead>Produtividade</TableHead><TableHead>Área</TableHead></TableRow>
              </TableHeader>
              <TableBody>
