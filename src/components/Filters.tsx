    ...
    <div className="flex flex-col h-full bg-card border-r border-border w-full max-w-xs overflow-hidden">
        ...
        <div className="p-4 space-y-6">
          <Accordion type="multiple" defaultValue={['years', 'states', 'metrics']} className="w-full">
            <AccordionItem value="years" className="border-none">
              <AccordionTrigger className="py-2 hover:no-underline">
                <div className="flex items-center gap-2"><Calendar className="w-4 h-4 text-accent" /><span className="text-sm font-medium">Série Temporal</span></div>
              </AccordionTrigger>
              <AccordionContent className="pt-2">
                <YearSelector years={availableYears} selected={filters.years} onChange={(y) => handleUpdate({ years: y })} />
              </AccordionContent>
            </AccordionItem>
            <Separator className="my-2 opacity-50" />
            <AccordionItem value="states" className="border-none">
              <AccordionTrigger className="py-2 hover:no-underline">
                <div className="flex items-center gap-2"><MapPin className="w-4 h-4 text-primary" /><span className="text-sm font-medium">Abrangência Regional</span></div>
              </AccordionTrigger>
              <AccordionContent className="pt-2">
                <StateSelector states={availableStates} selected={filters.states} onChange={(s) => handleUpdate({ states: s })} />
              </AccordionContent>
            </AccordionItem>
            <Separator className="my-2 opacity-50" />
            <AccordionItem value="metrics" className="border-none">
              <AccordionTrigger className="py-2 hover:no-underline">
                <div className="flex items-center gap-2"><TrendingUp className="w-4 h-4 text-secondary" /><span className="text-sm font-medium">Métricas de Eficiência</span></div>
              </AccordionTrigger>
              <AccordionContent className="pt-2 space-y-4">
                <MetricSelector current={filters.metric} onChange={(m) => handleUpdate({ metric: m })} />
                <RangeFilter value={filters.minProductivity} onChange={(v) => handleUpdate({ minProductivity: v })} />
              </AccordionContent>
            </AccordionItem>
          </Accordion>
          ...
          <div className="pt-4">
            <div className="flex items-center justify-between p-3 rounded-xl bg-primary/5 border border-primary/10">
              ...
              <Switch checked={filters.comparisonMode} onCheckedChange={(c) => handleUpdate({ comparisonMode: c })} />
            </div>
          </div>
        </div>
      </ScrollArea>
      ...
      <div className="p-4 border-t border-border bg-muted/10">
        <div className="flex flex-wrap gap-1">
          {filters.states.length > 0 && <Badge variant="secondary" className="bg-primary/10 text-primary border-none text-[10px] py-0">{filters.states.length} UFs</Badge>}
          {filters.years.length > 0 && <Badge variant="secondary" className="bg-accent/10 text-accent border-none text-[10px] py-0">{filters.years.length} Anos</Badge>}
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="grid grid-cols-3 gap-1.5">
      {[...years].sort((a, b) => b - a).map(year => (
        ...
        }} className={cn("px-2 py-1.5 rounded-md text-[11px] font-mono transition-all duration-200 border",
          selected.includes(year) ? "bg-accent text-accent-foreground border-accent" : "bg-background hover:bg-muted border-border text-muted-foreground")}>
          {year}
        </button>
      ))}
    </div>
  ...
  return (
    <div className="space-y-1">
      <div className="flex flex-wrap gap-1.5 mb-2">
        {selected.map(s => (
          <Badge key={s} variant="outline" className="gap-1 pl-2 pr-1 py-0.5 text-[10px] rounded-full bg-primary/5 border-primary/20">
            {s}<X className="w-3 h-3 cursor-pointer hover:text-destructive" onClick={() => onChange(selected.filter(x => x !== s))} />
          </Badge>
        ))}
      </div>
      <ScrollArea className="h-[180px] pr-2">
        <div className="space-y-1">
          {states.map(state => (
            <div key={state} className="flex items-center space-x-2 p-1.5 rounded-lg hover:bg-muted/50 transition-colors cursor-pointer"
              onClick={() => onChange(selected.includes(state) ? selected.filter(s => s !== state) : [...selected, state])}>
              <Checkbox id={`state-${state}`} checked={selected.includes(state)} onCheckedChange={() => {}}
                className="rounded-sm border-primary/50 data-[state=checked]:bg-primary" />
              <label className="text-xs font-medium leading-none cursor-pointer">{state}</label>
            </div>
          ))}
        </div>
      </ScrollArea>
      ...
      <div className="space-y-1.5">
        {metrics.map(m => (
          <button key={m.id} onClick={() => onChange(m.id)}
            className={cn("w-full flex items-center justify-between p-2.5 rounded-xl border transition-all duration-300",
              current === m.id ? "bg-primary/10 border-primary text-primary shadow-sm" : "bg-background border-border hover:border-primary/30")}>
...
export function RangeFilter({ value, onChange }: RangeFilterProps) {
  return (
    <div className="space-y-4 pt-2">
      ...
      <Slider value={[value]} max={5000} step={100} onValueChange={(v) => onChange(v[0])}
        className="[&_[role=slider]]:bg-primary [&_[role=slider]]:border-primary [&_[role=slider]]:h-4 [&_[role=slider]]:w-4" />
      <div className="flex justify-between text-[9px] text-muted-foreground font-mono"><span>0</span><span>2.5k</span><span>5k</span></div>
    </div>
  );
}
